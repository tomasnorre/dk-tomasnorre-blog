...
@section('body')
    @foreach ($posts->where('date', true)->take(1) as $featuredPost)
        <div class="relative group grid grid-cols-1 lg:grid-cols-12 gap-0 mb-6 overflow-hidden rounded-3xl bg-base shadow-md">
            ...
            <div class="lg:col-span-5 p-8 lg:p-12 flex flex-col justify-center">
                ...
                <a href="{{ $featuredPost->getUrl() }}" class="group/link">
                    <h2 class="leading-tight mb-4 transition-colors">
                        {{ $featuredPost->title }}
                    </h2>
                ...
                <div class="mt-auto">
                    <a href="{{ $featuredPost->getUrl() }}"
                       class="inline-flex items-center gap-2 font-bold link link-primary underline-offset-8 transition-colors decoration-emerald-500/30">
                        Read More →
                    </a>
                </div>
            </div>
